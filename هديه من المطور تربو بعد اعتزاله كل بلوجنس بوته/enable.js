var handler = async (m, { conn, usedPrefix, command, args, isOwner, isAdmin, isROwner }) => {
    ...
    // تعريف المتغير fake
    var fake = { key: { fromMe: false }, message: { conversation: 'Fake message' } }
    ...
    switch (type) {
        ...
        default:
            if (!/[01]/.test(command)) {
                return await conn.reply(m.chat, `*🚨 خيارات الجروبات*
...
${usedPrefix + command} *البوت_الفرعي*`, m)
    ...
    conn.reply(m.chat, `*${isEnable ? '🔓' : '🔒'} الخيار ${type}  ${isEnable ? 'شغال' : 'مقفول'} ل ${isAll ? 'هذا البوت' : isUser ? '' : 'هذا الدردشة.'}*`, m)
